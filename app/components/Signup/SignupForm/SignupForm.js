      ...
      <form className={styles.container} onSubmit={handleSubmit(submit)}>
        ...
        <div className={styles.nameContainer}>
          ...
          >
            ...
            />
            <p className={styles.error} style={{opacity: errors.first ? 1 : 0}}>* required field</p>
          </div>
          <div className={styles.inputContainer} style={{ marginRight: 0 }}>
            ...
            />
            <p className={styles.error} style={{opacity: errors.last ? 1 : 0}}>* required field</p>
          </div>
        </div>
        <div className={styles.inputContainer}>
          ...
          />
          <p className={styles.error} style={{opacity: errors.email ? 1 : 0}}>* required field</p>
        </div>
        <div className={styles.inputContainer}>
          ...
          />
          <p className={styles.error} style={{opacity: errors.password && errors.password?.type !== 'minLength' ? 1 : 0}}>* required field</p>
          <p className={styles.error} style={{opacity: errors.password?.type === 'minLength' ? 1 : 0}}>* password must be 8 characters</p>
        </div>
        <div
        ...
        >
          ...
          <div className={styles.input} style={{ cursor: "pointer" }}>
            <p className={styles.planSelected}>
              {selectedPlan}
              <HiChevronDown size={18} style={{transform: selectionContainer ? 'rotate(180deg)' : 'rotate(0deg)', transition: '400ms'}} />
            </p>
          </div>
          <div
          ...
          >
            <p
              ...
              onClick={() => setPlan(1)}
            >
              <BsCheckLg color="var(--gold)" style={{display: plan === 1 ? 'block' : 'none', marginRight: 10}} size={12} /> Free $0.00 /
              month
            </p>
            <p
              className={styles.selection}
              onClick={() => setPlan(2)}
            >
              <BsCheckLg color="var(--gold)" size={12} style={{display: plan === 2 ? 'block' : 'none', marginRight: 10}} /> Standard $8.99
              / month
            </p>
            <p
              className={styles.selection}
              onClick={() => setPlan(3)}
            >
              <BsCheckLg color="var(--gold)" size={12} style={{display: plan === 3 ? 'block' : 'none', marginRight: 10}} /> Premium $14.99
              / month
            </p>
          </div>
        </div>
        <div className={styles.btnContainer}>
          <button type="submit" className={styles.submitBtn}>{isLoading ? <ButtonLoader /> : buttonContent}</button>
        </div>
      </form>
    </div>
  );
